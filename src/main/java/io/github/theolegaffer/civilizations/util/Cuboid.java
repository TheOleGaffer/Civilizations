 ...
 * Taken from bukkit thread with desht and kingfarris
 */
...
public class Cuboid implements Cloneable, ConfigurationSerializable, Iterable<Block> {
    ...
    public Cuboid(Cuboid cuboid) {
        this(cuboid.worldName, cuboid.townName, cuboid.minimumPoint.getX(), cuboid.minimumPoint.getY(), cuboid.minimumPoint.getZ(), cuboid.maximumPoint.getX(), cuboid.maximumPoint.getY(), cuboid.maximumPoint.getZ());
    }
    ...
    public Cuboid(String worldName, String townName, double x1, double y1, double z1, double x2, double y2, double z2) {
    ...
    public String newSerialize(){
        return this.minimumPoint.getX() + ";" + this.maximumPoint.getX() + ";" + this.minimumPoint.getY() + ";" + this.maximumPoint.getY() + ";" +
                this.minimumPoint.getZ() + ";"  + this.maximumPoint.getZ() + ";" + this.worldName + ";" + this.townName;
    }

    public static Cuboid newDeserialize(String s) {
            String [] parts = s.split(";");
            double xPos1 = Double.parseDouble(parts[0]);
            double xPos2 = Double.parseDouble(parts[1]);
            double yPos1 = Double.parseDouble(parts[2]);
            double yPos2 = Double.parseDouble(parts[3]);
            double zPos1 = Double.parseDouble(parts[4]);
            double zPos2 = Double.parseDouble(parts[5]);
            String worldName = parts[6];
            String townName = parts[7];

            return new Cuboid(worldName, townName, xPos1, yPos1, zPos1, xPos2, yPos2, zPos2);
    }
    @Override
